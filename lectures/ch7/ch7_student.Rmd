...
# data hosted on data.world page 
...
```{r}
nhanes %>% 
  mutate(sex_n = ifelse(sex == "Male", 1, 0)) %>% # the latest output of this pipe is '.'
  lm(formula = height ~ sex_n, data = .) %>% 
  summary()
```
...
avocados = read_csv("https://query.data.world/s/zxsez6vfybv2fwaeorrgy3fufxsvnn")
```
...
If so, we should expect the demand curve shift will down for organic avocados. 
...
* The shift in the intercept from conventional to organic is -3.87157
...
Calculate average systolic blood pressure by sex:
